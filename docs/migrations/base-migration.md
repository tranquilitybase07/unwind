...
-- Production-ready, AI-friendly, scalable
-- ============================================================================
...
-- Insert the 7 fixed categories (do this after creating the table)
INSERT INTO categories (name, description, emoji, display_order, is_system) VALUES
('Tasks', 'Work projects, deadlines, actionable items', '✓', 1, true),
('Ideas', 'Creative thoughts, projects to explore', '💡', 2, true),
('Errands', 'Shopping, appointments, admin tasks', '🛒', 3, true),
('Health', 'Medical, fitness, nutrition, mental health', '❤️', 4, true),
('Relationships', 'People, communication, social obligations', '👥', 5, true),
('Worries Vault', 'Pure anxiety, intrusive thoughts, fears', '🔒', 6, true),
('Recurring', 'Regular routines, habits to build', '🔄', 7, true);
...
INDEX idx_user_processed (user_id, processing_status)
);
...
deadline_extracted_from_text TEXT, -- "by Friday", "tomorrow", "asap", etc.
...
worry_acknowledgment_text TEXT, -- For Worries Vault items
...
-- Relationships
blocked_by_item_id UUID REFERENCES items(id) ON DELETE SET NULL, -- For tasks blocked by other tasks
parent_task_id UUID REFERENCES items(id) ON DELETE SET NULL, -- For subtasks
...
INDEX idx_priority_score (user_id, final_priority_score DESC),
INDEX idx_status_created (status, created_at),
INDEX idx_is_spiral (user_id, is_worry_spiral),
...
INDEX idx_tag (tag),
INDEX idx_user_tag (tag, created_at)
);
...
-- Frequency details
days_of_week INTEGER[], -- [0,1,2,3,4,5,6] for Sun-Sat, e.g., [1,3,5] for Mon/Wed/Fri
...
reminder_location TEXT, -- e.g., "grocery store" for location-based
...
INDEX idx_user_task (user_id, ai_task),
...
INDEX idx_user_capture_time (user_id, capture_time)
);
...
-- ============================================================================
-- SUMMARY: 12 Tables + Views + Functions
-- ============================================================================
-- Core (MVP):
--   1. users
--   2. voice_dumps
...
--   12. sample_tags
--
-- Views: today, this_week, worries_vault (AI-friendly)
-- Functions: priority calculation, stats tracking
--
-- Total: Production-ready, scalable, AI-friendly
$$
